...
# High-confidence credential patterns. High-confidence only → legitimate prose ("rotate the password")
# doesn't trip; a real key format or `password = <secret>` does.
SECRET_PATTERNS = (
...
def scan_staged(home):
    ...
    for rel in (r.stdout.split("\0") if r.stdout else []):
        ...
        for pat in _SECRET_RE:
            m = pat.search(text)
            if m:
                hits.append((rel, m.group(0)[:24] + "…")); break
    return hits
